...
NUM_SIMS = 200 # adjust if too slow
INITIAL_MONEY = 1000000
NUM_YEARS = 2 # result will depend on years
STEP_SIZE = 5 # step size of permutations
ALLOWED_RISK = 0.15 # chance that you will lose any amount of money
...
def find_opt():
    ...
    # get all permutations of stock allocations
        ...
        for percent_B in range(0, 101 - percent_A, STEP_SIZE):
            for percent_C in range(0, 101 - percent_A - percent_B, STEP_SIZE):
                for percent_D in range(0, 101 - percent_A - percent_B - percent_C, STEP_SIZE):
                    # filter for permutations which sum to 100
                    if (percent_A + percent_B + percent_C + percent_D == 100):
                        # perform simulations with the current permutation
                        (mean, risk) = sim(percent_A, percent_B, percent_C, percent_D)
                        # search for the permutation which has the highest mean and lowest chance of risk
    ...
    profit = (highest_mean - INITIAL_MONEY) / INITIAL_MONEY * 100
    print(f'\t\t{portfolio}\n\t\tExpected Profit: ~{profit:.2f}%\n\t\tChance of Risk: ~{lowest_risk:.3f}%')
